...
package es.danirod.sprint9.game.entities;
...
import es.danirod.sprint9.game.Constants;
...
public class PlayerEntity extends Actor {
    ...
    public PlayerEntity(World world, Texture texture, Vector2 position) {
        ...
        setSize(es.danirod.sprint9.game.Constants.PIXELS_IN_METER, es.danirod.sprint9.game.Constants.PIXELS_IN_METER);
    }
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        // of the Box2D body.
        setPosition((body.getPosition().x - 0.5f) * es.danirod.sprint9.game.Constants.PIXELS_IN_METER,
                    (body.getPosition().y - 0.5f) * es.danirod.sprint9.game.Constants.PIXELS_IN_METER);
    ...
    public void act(float delta) {
        ...
        if (alive) {
            ...
            if (es.danirod.sprint9.game.Constants.GAME_MODE == 0) {
                body.setLinearVelocity(Constants.PLAYER_SPEED_EASY, speedY);
            }else if (es.danirod.sprint9.game.Constants.GAME_MODE == 1) {
                body.setLinearVelocity(Constants.PLAYER_SPEED_MEDIUM, speedY);
            }else if (es.danirod.sprint9.game.Constants.GAME_MODE == 2) {
                body.setLinearVelocity(Constants.PLAYER_SPEED_HARD, speedY);
            }
        }

        // If the player is jumping, apply some opposite force so that the player falls faster.
        if (jumping) {
            body.applyForceToCenter(0, -es.danirod.sprint9.game.Constants.IMPULSE_JUMP * 1.15f, true);
        }
    }
    ...
    public void jump() {
        ...
        if (!jumping && alive) {
            ...
            body.applyLinearImpulse(0, es.danirod.sprint9.game.Constants.IMPULSE_JUMP, position.x, position.y, true);
        }
    }
